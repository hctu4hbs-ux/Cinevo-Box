...
// Streaming Sources Configuration - Low-ad/Ad-free alternatives
const STREAMING_SOURCES = {
    vidsrc: 'https://vidsrc.me/embed/',
    vidlink: 'https://vidlink.pro/embed/movie/',
    multiembed: 'https://multiembed.mov/directstream.php?video_id=',
    autoembed: 'https://www.autoembed.cc/embed/imdb/',
    warezcdn: 'https://warezcdn.com/embed/',
    moviesdotto: 'https://movies.do/embed/'
};
...
function generateStreamingLinks(imdbId, title, mediaType = 'movie') {
    ...
    if (imdbId) {
        ...
        links.vidlink = `${STREAMING_SOURCES.vidlink}${cleanImdbId}`;
        links.multiembed = `${STREAMING_SOURCES.multiembed}${cleanImdbId}`;
        links.autoembed = `${STREAMING_SOURCES.autoembed}${cleanImdbId}`;
        links.warezcdn = `${STREAMING_SOURCES.warezcdn}${cleanImdbId}`;
        links.moviesdotto = `${STREAMING_SOURCES.moviesdotto}${cleanImdbId}`;
        
        // For TV shows, append season/episode info
        if (mediaType === 'tv') {
            links.vidsrc += '?s=1&e=1';
            links.vidlink += '?s=1&e=1';
            links.multiembed += '&s=1&e=1';
            links.autoembed += '?s=1&e=1';
            links.warezcdn += '?s=1&e=1';
            links.moviesdotto += '?s=1&e=1';
        }
    }
